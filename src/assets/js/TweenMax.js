      ...
      _calculateControlPoints = function (
        ...
        for (i = 0; i < l; i++) {
          ...
          if (correlate) {
            ...
            mm =
              p2 - (m1 + (((m2 - m1) * ((r1 * 3) / (r1 + r2) + 0.5)) / 4 || 0));
          } else {
        ...
        set: function (v) {
          ...
          if (!this._timeRes) {
            ...
            t =
              v === 1
                ? 1
                : (i + (v1 - this._s1) / (this._s2 - this._s1)) * this._prec ||
                  0;
          }
          inv = 1 - t;
    ...
    function (TweenPlugin, TweenLite) {
      ...
      var _numExp = /(?:\-|\.|\b)(\d|\.|e\-)+/g,
        ...
        _createElement = function (type, ns) {
          ...
          return e.style ? e : _doc.createElement(type); //some environments won't allow access to the element's style when created with a namespace in which case we default to the standard createElement() to work around the issue. Also note that when GSAP is embedded directly inside an SVG file, createElement() won't allow access to the style object in Firefox (see https://greensock.com/forums/topic/20245-problem-using-tweenmax-in-standalone-self-containing-svg-file-err-cannot-set-property-csstext-of-undefined/).
        },
        ...
        _getTransform = (_internals.getTransform = function (
          ...
          if (m !== _identity2DMatrix) {
            if (m.length === 16) {
              ...
              //scales
              tm.scaleX =
                ((Math.sqrt(a11 * a11 + a21 * a21 + a31 * a31) * rnd + 0.5) |
                  0) /
                rnd;
              tm.scaleY =
                ((Math.sqrt(a22 * a22 + a32 * a32) * rnd + 0.5) | 0) / rnd;
              tm.scaleZ =
                ((Math.sqrt(a13 * a13 + a23 * a23 + a33 * a33) * rnd + 0.5) |
                  0) /
                rnd;
              a11 /= tm.scaleX;
              a12 /= tm.scaleY;
